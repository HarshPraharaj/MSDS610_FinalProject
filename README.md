...
### Nested Loops Join
It is simplest form of join, which compares each row from one table (known as the outer table) to each row from the other table (known as the inner table) looking for rows that satisfy the join predicate. 
...
For more details on installation see the pandas installation page.

https://pandas.pydata.org/docs/getting_started/install.html
___

## How to Use this Project
This project assumes the user has the com_data.csv, address.csv, and emps.csv files downloaded into your current working directory for Python. If this is not the case, pandas will throw an error. To ensure this does not happen specify the proper pathing in any ```pd.csv_read("your file location")``` statement. In addition it is possible to use this project as starter code to join your own data. To do so, alter the read csv statement to ```pd.read_csv("your new file")``` and change the primary and foriegn keys to the proper names in the pd.merge statement. For example to do an inner join:


``` pd.merge(table1,table2, on = "your_key", how = 'inner') ```


In this way this project can be used on any number of different datasets as needed. **Keep in mind that each dataset is different and may require various data cleansing, preparation, or techniques not covered in this project.** 
